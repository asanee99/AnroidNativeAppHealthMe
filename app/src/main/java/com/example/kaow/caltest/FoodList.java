 ...
 */
public class FoodList extends AsyncTask<Void, Void, String> {
    ...
    public FoodList(Context context, String url) {
        super();
        mUrl = url;
        mContext = context;
        ...
        try {
            ...
            if (success == 1) {
                ...
                for (int i = 0; i < food.length(); i++) {
                    ...
                    ((ShowFoodList) mContext).list.setAdapter(adapter);
                }
            } else if (success == 0) {
                ((ShowFoodList) mContext).list.setAdapter(null);
